...
const Home = () => {
  ...
  const like_unlike_effect = (element_id, cssClass) => {
    const like_unlike_effect_icon = document.getElementById(
      element_id + "-like-effect"
    );
  ...
  const makeComment = (text, postId) => {
    if (!text) {
      M.toast({
        html: `⚠️<span style="color:black" > Comment can't be Empty...</span>`,
        classes: "yellow red accent-2",
      });
      return;
    }
  ...
  const deletePost = (postId) => {
    ...
    M.toast({
      html: `⚠️<span style="color:black" > Post Deleted</span>`,
      classes: "yellow red accent-2",
    })
    .then((result) => {
      // console.log(result);
  ...
  const deleteComment = (postid, commentid) => {
    ...
    M.toast({
      html: `⚠️<span style="color:black" > Comment Deleted</span>`,
      classes: "yellow red accent-2",
    })
    .then((result) => {
      // console.log(result);
  ...
  return (
    <div className="home">
      ...
      {data.map((item, index) => {
        const hasThreeOrMoreComments = item.comments.length >= 3;
        return (
          <div className="card home-card" key={item._id}>
              ...
              </BootstrapTooltip>
              {
                ...
                item.postedBy._id === state._id && (
                  <BootstrapTooltip placement="left" title="Delete Post" arrow>
                    <div style={{ color: "grey", paddingLeft: "480px" }}>
                      <span
                        className="material-icons deletepost"
            ...
            <div className="card-image">
              <i className="material-icons" id={item._id + "-like-effect"}>
                favorite
              </i>
              <img
                alt=""
                ...
                onDoubleClick={() => {
                  item.likes.includes(myInfo._id)
                    ? unlikePost(item._id)
                    : likePost(item._id);
                }}
              />
            ...
            <div className="card-content">
              ...
              <h6>{item.title}</h6>
              <h6>
                <span style={{ fontWeight: "lighter" }}>{item.body}</span>
              </h6>
              <br />
              <p>Comments</p>
              ...
              <div
                className="comments"
                style={
                  hasThreeOrMoreComments
                    ? { overflowY: "scroll", maxHeight: "100px" }
                    : {}
                }
              >
              ...
              <div className="commentbox">
                ...
                >
                  <span
                    ...
                    onClick={() => {
                      makeComment(
                        document.getElementById(`comment-post-${item._id}`)
                          .value,
                        item._id
                      );
                      document.getElementById(
                        `comment-post-${item._id}`
                      ).value = "";
                    }}
                  >
                    send
